...
use Jjpsos\ContractPilot\Foundation\Interfaces\Scriptable;
 ...
 * @license GPL-3.0+
 */
class AssetRegistry implements Scriptable
{
    /**
     * The plugin instance.
     *
    ...
    public function register_script(
        ...
        if (!preg_match("/^(http|https):\/\//", $src)) {
            ...
            $url = $src;
            $path = str_replace(
                $this->plugin->get_dir_url(),
                $this->plugin->get_dir_path(),
                $src,
            );
        }
    ...
    public function enqueue_script(
        ...
        if (wp_scripts()->query($handle)) {
            ...
            if (
                isset($this->localized[$handle]) &&
                is_array($this->localized[$handle])
            ) {
    ...
    public function add_data($handle, $name, $data)
    {
        ...
        if (did_action("wp_enqueue_scripts")) {
            _doing_it_wrong(
                __METHOD__,
                esc_html(
                    "The script has already been enqueued. Please localize the script before enqueuing it.",
                ),
                "1.0.0",
            );
            return;
