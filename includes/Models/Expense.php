...
class Expense extends Transaction
{
    ...
    public function save()
    {
        if (empty($this->payment_date)) {
            return new \WP_Error('missing_required', __('Expense date is required.', 'contract-pilot'));
        }
        if (empty($this->account_id)) {
            return new \WP_Error('missing_required', __('Account is required.', 'contract-pilot'));
        }
    ...
    public function get_next_number()
    {
        ...
        $prefix = get_option('contract_pilot_expense_prefix', strtoupper(substr($this->get_object_type(), 0, 3)) . '-');
        $number = str_pad($max + 1, get_option('contract_pilot_expense_digits', 4), '0', STR_PAD_LEFT);
    ...
    {
        return admin_url('admin.php?page=contract-pilot-purchases&tab=expenses&action=edit&id=' . $this->id);
    }


    public function get_view_url()
    {
        return admin_url('admin.php?page=contract-pilot-purchases&tab=expenses&action=view&id=' . $this->id);
    }
