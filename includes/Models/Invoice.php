...
class Invoice extends Document
{
    ...
    {
        $due_after = get_option("contract_pilot_invoice_due_date", 7);
        $this->attributes = array_merge($this->attributes, [
            "type" => $this->get_object_type(),
            "issue_date" => current_time("mysql", true),
            "due_date" => wp_date(
                "Y-m-d",
                strtotime("+" . $due_after . " days"),
                new \DateTimeZone("UTC"),
            ),
            "note" => get_option("contract_pilot_invoice_note", ""),
    ...
    {
        $mapped = contract_pilot_invoice_status_label_for_status((string) $this->status);
        if ("" !== $mapped) {
            return $mapped;
        }
        $statuses = contract_pilot()->invoices->get_statuses();

        return array_key_exists($this->status, $statuses)
    ...
    {
        return $this->has_many(Note::class, "parent_id")->set(
            "parent_type",
            "invoice",
        );
    }
    ...
    {
        $auto_number_assigned = false;
        if (empty($this->number)) {
            $this->number = $this->get_next_number();
            $auto_number_assigned = true;
        }
        ...
        if (
            !in_array(
                $this->status,
                ["paid", "partial", "overdue", "accept", "otto"],
                true,
            )
        ) {
        ...
        return $this->save_with_number_retry($auto_number_assigned);
    }
    ...
    public function set_items($items)
    {
        ...
        foreach ($items as $i => &$itemdata) {
            $quantity = isset($itemdata["quantity"])
                ...
                : 1;
            $item_id = isset($itemdata["item_id"])
                ? absint($itemdata["item_id"])
                : 0;
            $item = contract_pilot()->items->get($item_id);
            ...
            $itemdata["name"] = isset($itemdata["name"])
                ? sanitize_text_field($itemdata["name"])
                : $item->name;
            $itemdata["description"] = isset($itemdata["description"])
                ? sanitize_text_field($itemdata["description"])
                : $item->description;
            $itemdata["unit"] = isset($itemdata["unit"])
                ? sanitize_text_field($itemdata["unit"])
                : $item->unit;
            $itemdata["type"] = isset($itemdata["type"])
                ? sanitize_text_field($itemdata["type"])
                : $item->type;
            $itemdata["price"] = isset($itemdata["price"])
            ...
            if (
                array_key_exists("taxes", $itemdata) &&
                is_array($itemdata["taxes"])
            ) {
                foreach ($itemdata["taxes"] as $j => &$taxdata) {
                    if (!is_array($taxdata) || empty($taxdata)) {
                        continue;
                    }
                    $taxdata["tax_id"] = isset($taxdata["tax_id"])
                        ? absint($taxdata["tax_id"])
                        : 0;
                    $tax = contract_pilot()->taxes->get($taxdata["tax_id"]);

                    if (!$tax) {
                        unset($itemdata["taxes"][$j]);
                        continue;
                    }
                    $taxdata["name"] = isset($taxdata["name"])
                        ? sanitize_text_field($taxdata["name"])
                        : $tax->name;
                    $taxdata["rate"] = isset($taxdata["rate"])
                        ? floatval($taxdata["rate"])
                        : $tax->rate;
                    $taxdata["amount"] = 0;
                }
            }
            ...
            $items_total +=
                "standard" === $itemdata["type"] ? $itemdata["subtotal"] : 0;
        }


        $discount =
            "percentage" === $this->discount_type
                ? ($items_total * $this->discount_value) / 100
                : $this->discount_value;
        $discount = min($discount, $items_total);
        foreach ($items as $item) {
            $item["discount"] =
                "standard" === $item["type"]
                    ? ($discount / $items_total) * $item["subtotal"]
                    : 0;
    ...
    public function get_next_number()
    {
        ...
        $prefix = get_option(
            "contract_pilot_invoice_prefix",
            strtoupper(substr($this->get_object_type(), 0, 3)) . "-",
        );
        $number = str_pad(
            $max + 1,
            get_option("contract_pilot_invoice_digits", 4),
            "0",
            STR_PAD_LEFT,
        );
    ...
    public function get_paid_amount()
    {
        ...
        foreach ($this->payments as $payment) {
            $paid += contract_pilot_convert_currency(
                $payment->amount,
        ...
        return round($paid, contract_pilot()->currencies->get_precision($this->currency));
    }
    ...
    public function calculate_totals()
    {
        ...
        if ($paid_amount > 0 && $due_amount > 0) {
            ...
            in_array($this->status, ["paid", "partial", "accept"], true) &&
            $paid_amount <= 0 &&
            "overdue" !== $this->status
        ) {
            $this->status = "sent";
    ...
    {
        return "yes" === get_option("contract_pilot_tax_enabled", "no") ||
            ($this->exists() && $this->tax > 0);
    }
    ...
    {
        return !(empty($this->due_date) || $this->is_paid()) &&
            strtotime(date_i18n("Y-m-d 23:59:00")) >
                strtotime(
                    date_i18n("Y-m-d 23:59:00", strtotime($this->due_date)),
                );
    }
    ...
    {
        return admin_url(
            "admin.php?page=contract-pilot-sales&tab=invoices&action=edit&id=" . $this->id,
        );
    }
    ...
    {
        return admin_url(
            "admin.php?page=contract-pilot-sales&tab=invoices&action=view&id=" . $this->id,
        );
    }
    ...
    {
        return site_url("contract-pilot/invoice/?uuid=" . $this->uuid);
    }
}
