...
{
    protected $table = 'pilot_transfers';
    ...
    public function save()
    {
        ...
        if (! $from_account || ! $to_account) {
            return new \WP_Error('invalid_account', __('Invalid account.', 'contract-pilot'));
        }


        if ($from_account_id === $to_account_id) {
            return new \WP_Error('same_account', __('From and to accounts cannot be the same.', 'contract-pilot'));
        }


        if (! is_numeric($this->amount)) {
            return new \WP_Error('invalid_amount', __('Invalid amount.', 'contract-pilot'));
        }
        ...
        if (! empty($this->from_exchange_rate)) {
            ...
            $from_rate = floatval(contract_pilot()->currencies->get_rate($from_account->currency));
        }
        ...
        if (! empty($this->to_exchange_rate)) {
            ...
            $to_rate = floatval(contract_pilot()->currencies->get_rate($to_account->currency));
        }
        ...
        if ($from_account->currency !== $to_account->currency) {
            $amount = contract_pilot_convert_currency($amount, $from_rate, $to_rate);
        }
    ...
    {
        return admin_url('admin.php?page=contract-pilot-banking&tab=transfers&action=edit&id=' . $this->id);
    }


    public function get_view_url()
    {
        return admin_url('admin.php?page=contract-pilot-banking&tab=transfers&action=view&id=' . $this->id);
    }
}
