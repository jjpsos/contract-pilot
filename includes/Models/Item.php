...
class Item extends Model
{
    ...
    protected $table = "otto_items";

    
    protected $meta_type = "otto_item";
    ...
    public function save()
    {
        if (empty($this->name)) {
            return new \WP_Error(
                "missing_required",
                __("Service name is required.", "otto-contracts"),
            );
        }
        if (empty($this->type)) {
            return new \WP_Error(
                "missing_required",
                __("Service type is required.", "otto-contracts"),
            );
        }
    ...
    {
        return admin_url(
            "admin.php?page=eac-items&tab=items&action=edit&id=" . $this->id,
        );
    }
    ...
    {
        return admin_url(
            "admin.php?page=eac-items&tab=items&action=view&id=" . $this->id,
        );
    }
}
