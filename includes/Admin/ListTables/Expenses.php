...
class Expenses extends ListTable
{
    public function __construct($args = array())
    {
        ...
        $this->base_url = Request::list_table_url(
            admin_url('admin.php?page=contract-pilot-purchases&tab=expenses'),
        );
    }
    ...
    public function prepare_items()
    {
        ...
        $per_page    = $this->get_items_per_page('contract_pilot_expenses_per_page', 20);
        ...
        $args = apply_filters('contract_pilot_expenses_table_query_args', $args);
    ...
    {
        if (! current_user_can('contract_pilot_delete_expenses')) {
            contract_pilot()->flash->error(__('You do not have permission to delete expenses.', 'contract-pilot'));
            return;
        }

        $performed = 0;
        foreach ($ids as $id) {
            if (contract_pilot()->expenses->delete($id)) {
                ++$performed;
            }
        }
        if (! empty($performed)) {
            contract_pilot()->flash->success(
                sprintf(
                    /* translators: %s: number of expenses deleted (formatted) */
                    __('%s expense(s) deleted successfully.', 'contract-pilot'),
    ...
    {
        esc_html_e('No expenses found.', 'contract-pilot');
    }
    ...
    protected function extra_tablenav($which)
    {
        ...
        if ('top' === $which) {
            $months = DatabaseUtil::get_results_list_table_year_month_transaction_filters('expense');

            $this->date_filter($months);
            $this->account_filter();
            $this->category_filter('expense');
            submit_button(__('Filter', 'contract-pilot'), '', 'filter_action', false);
        }
        echo '</div>';
    }
    ...
    public function get_columns()
    {
        return array(
            ...
            'number'     => __('Expense #', 'contract-pilot'),
    ...
    {
        return $item->payment_date ? contract_pilot_format_datetime($item->payment_date, contract_pilot_date_format()) : '&mdash;';
    }
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        if (! $item->editable || ! current_user_can('contract_pilot_edit_expenses')) {
            unset($actions['edit']);
        }
