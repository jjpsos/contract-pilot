...
class Transfers extends ListTable
{
    public function __construct($args = [])
    {
        ...
        $this->base_url = Request::list_table_url(
            admin_url("admin.php?page=contract-pilot-banking&tab=transfers"),
        );
    }
    ...
    public function prepare_items()
    {
        ...
        $per_page = $this->get_items_per_page("contract_pilot_transfers_per_page", 20);
        ...
        $args = apply_filters("contract_pilot_transfers_table_query_args", $args);
    ...
    {
        if (!current_user_can("contract_pilot_delete_transfers")) {
        ...
        $performed = 0;
        foreach ($ids as $id) {
            if (contract_pilot()->transfers->delete($id)) {
                ++$performed;
            }
        }
        if (!empty($performed)) {
            contract_pilot()->flash->success(
                sprintf(
                    /* translators: %s: number of transfers deleted (formatted). */
                    __(
    ...
    {
        esc_html_e("No transfers found.", "contract-pilot");
    }
    ...
    protected function extra_tablenav($which)
    {
        ...
        if ("top" === $which) {
            $months = DatabaseUtil::get_results_list_table_year_month_transfers();
    ...
    public function get_columns()
    {
        return [
            ...
            "transfer_date" => __("Date", "contract-pilot"),
            "from_account_id" => __("From Account", "contract-pilot"),
            "to_account_id" => __("To Account", "contract-pilot"),
    ...
    public function column_transfer_date($item)
    {
        return sprintf(
            ...
            esc_html(
                $item->transfer_date
                    ? contract_pilot_format_datetime(
                        $item->transfer_date,
                        contract_pilot_date_format(),
                    )
                    : "&mdash;",
            ),
        );
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        if (!current_user_can("contract_pilot_edit_transfers")) {
            unset($actions["edit"]);
        }
