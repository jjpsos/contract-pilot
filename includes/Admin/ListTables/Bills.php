...
class Bills extends ListTable {
	...
	protected function bulk_set_draft( $ids ) {
		if ( ! current_user_can( 'eac_edit_bills' ) ) { 
			EAC()->flash->error( __( 'You do not have permission to perform this action.', 'otto-contracts' ) );
			return;
		}
		...
		if ( ! empty( $performed ) ) {
			...
			EAC()->flash->success( sprintf( __( '%s bill(s) marked as draft successfully.', 'otto-contracts' ), number_format_i18n( $performed ) ) );
		}
	}
	...
	protected function bulk_set_received( $ids ) {
		...
		if ( ! empty( $performed ) ) {
			...
			EAC()->flash->success( sprintf( __( '%s bill(s) marked as received successfully.', 'otto-contracts' ), number_format_i18n( $performed ) ) );
		}
	}
	...
	protected function bulk_set_overdue( $ids ) {
		...
		if ( ! empty( $performed ) ) {
			...
			EAC()->flash->success( sprintf( __( '%s bill(s) marked as overdue successfully.', 'otto-contracts' ), number_format_i18n( $performed ) ) );
		}
	}
	...
	protected function bulk_set_cancelled( $ids ) {
		...
		if ( ! empty( $performed ) ) {
			...
			EAC()->flash->success( sprintf( __( '%s bill(s) marked as cancelled successfully.', 'otto-contracts' ), number_format_i18n( $performed ) ) );
		}
	}
	...
	protected function bulk_delete( $ids ) {
		if ( ! current_user_can( 'eac_delete_bills' ) ) { 
			EAC()->flash->error( __( 'You do not have permission to delete bills.', 'otto-contracts' ) );
			return;
		}
		...
		if ( ! empty( $performed ) ) {
			...
			EAC()->flash->success( sprintf( __( '%s bill(s) deleted successfully.', 'otto-contracts' ), number_format_i18n( $performed ) ) );
		}
	}
	...
	protected function get_bulk_actions() {
		...
		if ( current_user_can( 'eac_edit_bills' ) ) { 
			...
			$actions['set_cancelled'] = __( 'Set Cancelled', 'otto-contracts' );
		}
	...
	public function column_issue_date( $item ) {
		...
		$metadata = $item->due_date ? sprintf(  __( 'Due: %s', 'otto-contracts' ), eac_format_datetime( $item->due_date, eac_date_format() ) ) : '';
