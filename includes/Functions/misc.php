...
function eac_form_field( $field ) {
	...
	switch ( $field['type'] ) {
		...
		case 'file':
			$field = wp_parse_args(
				...
				array(
					'button_class' => '',
					'button_label' => __( 'Choose File', 'otto-contracts' ),
					'mime_types'   => '',
				)
			);
			$file  = array(
	...
	if ( ! empty( $input ) && ! empty( $field['label'] ) ) {
		$required = true === $field['required'] ? '&nbsp;<abbr title="' . esc_attr__( 'required', 'otto-contracts' ) . '"></abbr>' : '';
...
function eac_file_uploader( $field ) {
	$field = wp_parse_args(
		...
		array(
			'value'        => '',
			'name'         => 'attachment_id',
			'button_label' => __( 'Upload attachment', 'otto-contracts' ),
			'mime_types'   => '',
			'icon_url'     => '',
			'title'        => '',
			'src'          => '',
	...
	if ( $field['readonly'] && empty( $field['value'] ) ) {
		echo esc_html__( 'No file uploaded', 'otto-contracts' );
		return;
	}
