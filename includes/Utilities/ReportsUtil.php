...
class ReportsUtil
{
    ...
    {
        $options = [
            "today" => __("Today", "contract-pilot"),
            "yesterday" => __("Yesterday", "contract-pilot"),
            "this_week" => __("This Week", "contract-pilot"),
            "last_week" => __("Last Week", "contract-pilot"),
            "last_30_days" => __("Last 30 Days", "contract-pilot"),
            "this_month" => __("This Month", "contract-pilot"),
            "last_month" => __("Last Month", "contract-pilot"),
            "this_quarter" => __("This Quarter", "contract-pilot"),
            "last_quarter" => __("Last Quarter", "contract-pilot"),
            "this_year" => __("This Year", "contract-pilot"),
            "last_year" => __("Last Year", "contract-pilot"),
            "custom" => __("Custom", "contract-pilot"),
        ];

        return apply_filters("contract_pilot_dates_filter_options", $options);
    }
    ...
    public static function get_random_color($key = null)
    {
        ...
        $colors = apply_filters("contract_pilot_report_colors", [
            "#3366cc",
            "#dc3912",
            "#ff9900",
            "#109618",
    ...
    {
        $reports = get_transient("contract_pilot_payments_report");
        ...
        if ($force || empty($reports[$year])) {
            $transactions = DatabaseUtil::get_results_payments_report_transaction_rows(
            ...
            $reports[$year] = apply_filters(
                "contract_pilot_payments_report",
                $data,
                $year,
            );

            set_transient("contract_pilot_payments_report", $reports, HOUR_IN_SECONDS);
        }
    ...
    {
        $reports = get_transient("contract_pilot_expenses_report");
        $reports = !is_array($reports) ? [] : $reports;
        $year = empty($year) ? wp_date("Y") : $year;
        $start_date = self::get_year_start_date($year);
        $end_date = self::get_year_end_date($year);
        ...
        if (
            ...
            $transactions = DatabaseUtil::get_results_expenses_report_transaction_rows(
                get_gmt_from_date($start_date),
                get_gmt_from_date($end_date),
            );
            ...
            $reports[$year] = apply_filters(
                "contract_pilot_expenses_report",
                $data,
                $year,
            );

            set_transient("contract_pilot_expenses_report", $reports, HOUR_IN_SECONDS);
        }
    ...
    {
        $reports = get_transient("contract_pilot_profits_report");
        $reports = !is_array($reports) ? [] : $reports;
        $year = empty($year) ? wp_date("Y") : $year;
        $start_date = self::get_year_start_date($year);
        $end_date = self::get_year_end_date($year);
        ...
        if (
            ...
            $transactions = DatabaseUtil::get_results_profits_report_transaction_rows(
                get_gmt_from_date($start_date),
                get_gmt_from_date($end_date),
            );
            ...
            $reports[$year] = apply_filters("contract_pilot_profits_report", $data, $year);

            set_transient("contract_pilot_profits_report", $reports, HOUR_IN_SECONDS);
        }
    ...
    /**
     * @param string $column One of the allowlisted column identifiers.
     * @return string
     */
    public static function sql_localized_datetime_expression($column)
    {
        return DatabaseUtil::sql_localized_datetime_expression($column);
    }
