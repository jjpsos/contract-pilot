...
use Otto\ByteKit\Models\Traits\HasAttributes;
use Otto\ByteKit\Models\Traits\HasMetaData;
use Otto\ByteKit\Models\Traits\HasRelations;
/**
...
abstract class Model
{
    ...
    protected function booting()
    {
        ...
        if (empty($this->columns)) {
            $this->columns = $wpdb->get_col(
                "DESCRIBE {$wpdb->prefix}{$this->table}",
            );
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Prepared.
        }
        // Setup primary key.
    ...
    */
    ...
    protected function read($id)
    {
        ...
        if (false === $data) {
            ...
            foreach ($args as $key => $value) {
                if (is_array($value)) {
                    ...
                    $where[] = $wpdb->prepare("`{$key}` = %d", $value);
                    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Prepared.
                } else {
            ...
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Prepared in the previous line.
        ...
        $attributes = apply_filters(
            "{$this->get_hook_prefix()}_attributes",
            $data,
            $data,
        );
        ...
        foreach ($attributes as $key => $value) {
            ...
            $attributes[$key] = apply_filters(
                "{$this->get_hook_prefix()}_attribute_{$key}",
                $value,
                $attributes,
            );
        }
    ...
    public function save()
    {
         ...
         */
        $check = apply_filters(
            "{$this->get_hook_prefix()}_check_save",
            null,
            $this,
        );
        if (null !== $check) {
        ...
        foreach ($this->get_transitionable() as $transitionable) {
            ...
            if ($old_value !== $new_value) {
                 ...
                 */
                do_action(
                    "{$this->get_hook_prefix()}_pre_{$transitionable}_transition",
                    $this,
                    $new_value,
                    $old_value,
                );
                 ...
                 */
                do_action(
                    "{$this->get_hook_prefix()}_pre_{$transitionable}_{$new_value}",
                    $this,
                    $old_value,
                );
            }
        ...
        if (!$this->exists()) {
             ...
             */
            $data = apply_filters(
                "{$this->get_hook_prefix()}_insert_data",
                $data,
                $this,
            );
             ...
             * @since 1.0.0
             */
            $updates = apply_filters(
                "{$this->get_hook_prefix()}_update_data",
                $updates,
                $this,
            );
        ...
        foreach ($this->get_transitionable() as $transitionable) {
            ...
            if ($this->is_dirty($transitionable)) {
                 ...
                 */
                do_action(
                    "{$this->get_hook_prefix()}_{$transitionable}_transition",
                    $this,
                    $new_value,
                    $old_value,
                );
                 ...
                 */
                do_action(
                    "{$this->get_hook_prefix()}_{$transitionable}_{$new_value}",
                    $this,
                    $old_value,
                );
            }
    ...
    public function delete()
    {
         ...
         */
        $check = apply_filters(
            "{$this->get_hook_prefix()}_check_delete",
            null,
            $this,
        );
        if (null !== $check) {
