...
namespace Otto\ByteKit\Admin;

use Otto\ByteKit\Plugin;
...
class Notices
{
    ...
    public function __construct($plugin)
    {
        $this->plugin = $plugin;
        add_action(
            "wp_ajax_" . $this->plugin->get_prefix() . "_dismiss_notice",
    ...
    public function ajax_dismiss_notice()
    {
        if (
            !check_ajax_referer(
                $this->plugin->get_prefix() . "_dismiss_notice",
                "nonce",
                false,
            ) ||
        ...
        $notice_id = isset($_POST["notice_id"])
            ? sanitize_text_field(wp_unslash($_POST["notice_id"]))
            : "";
        $snooze = isset($_POST["snooze"])
            ? filter_var(wp_unslash($_POST["snooze"]), FILTER_VALIDATE_BOOLEAN)
            : false;
        $snooze_time = isset($_POST["snooze_time"])
            ? absint(wp_unslash($_POST["snooze_time"]))
            : 7 * DAY_IN_SECONDS;
        $notice = array_key_exists($notice_id, $this->notices)
        ...
        foreach ($this->notices as $notice) {
            if ($this->should_display($notice)) {
                $classes = array_unique(
                    array_filter(wp_parse_list($notice["class"])),
                );
    ...
    public function add($args)
    {
        ...
        if (empty($args["notice_id"])) {
            $args["notice_id"] =
                $this->plugin->get_prefix() .
                "_" .
                md5($args["message"] . $args["type"]);
        }
        // if dismissible and already dismissed, don't add the notice.
        if (
            true ===
                filter_var($args["dismissible"], FILTER_VALIDATE_BOOLEAN) &&
            $this->is_dismissed($args["notice_id"])
        ) {
            return;
        }
    ...
    public function should_display($notice)
    {
        if (
            ($notice["notice_id"] &&
                $this->is_dismissed($notice["notice_id"])) ||
            ($notice["capability"] && !current_user_can($notice["capability"]))
        ) {
            return false;
        }
        return true;
